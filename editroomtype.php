...
<body>
	<div class="container">
		...
		<?php
			...
			$find_roomtype = "SELECT * FROM roomtypes WHERE id= '$roomtype_id'";
			$select_roomtype = mysqli_query($connect, $find_roomtype);
			$count_roomtypes = mysqli_num_rows($select_roomtype);
			$room_type = mysqli_fetch_assoc($select_roomtype);
		?>

		<!--Edit Guest form Form-->
		<div class="w3-content well" style="margin-top: 100px; background-color: white; max-width: 700px;">
			<h4 class="w3-text white">Edit Room type</h4>
			<form method="POST" action="edit_roomtype.php">
				<input type="hidden" name="id" value= "<?php echo $roomtype_id; ?>">
					<div class="form-group">
						<label>Type</label>
						<input type="text" name="type"  class="form-control" id="firstname" value= "<?php echo $room_type['type']; ?>">
